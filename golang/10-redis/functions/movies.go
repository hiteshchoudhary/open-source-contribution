...
func AddMovie(w http.ResponseWriter, r *http.Request){
	...
	objectIdString := result.InsertedID.(primitive.ObjectID).Hex()
	newRating := int(newMovie["rating"].(float64))

	fmt.Printf("Inserted document with _id: %v\n", result.InsertedID)
	AddMovieInRedis(RedisMovie{Id: objectIdString, Rating: newRating})
}